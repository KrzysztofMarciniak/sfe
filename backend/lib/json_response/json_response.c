 ...
 *
 * Frees any existing message and sets a new one by duplicating the input string.
 * If msg is NULL, the existing message is freed and set to NULL.
 *
 ...
 *
 * @param resp Pointer to the JsonResponse instance.
 * @return Const pointer to the generated JSON string, or a static error JSON on failure.
 */
 ...
 *
 * Safely deallocates the message, JSON object, and the JsonResponse struct itself.
 *
 ...
 *
 * Allocates and initializes a JsonResponse instance and wires up method pointers.
 *
 * @param status The HTTP status code (e.g., 200, 400).
 * @return Pointer to the newly created JsonResponse, or NULL on allocation failure.
 */
