...
result_t* result_new_success(void) {
        ...
        r->error.failed_file = NULL;
        r->error.failed_func = NULL;
        r->error.extra_info  = NULL;
        return r;
}
...
static result_t* result_new_impl(result_code_t rc, const char* message,
        ...
        r->error.message     = safe_strdup(message);
        r->error.failed_file = safe_strdup(failed_file);
        r->error.failed_func = safe_strdup(failed_func);
        r->error.extra_info  = safe_strdup(extra_info);
